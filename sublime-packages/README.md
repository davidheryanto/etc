...
1. **Openers** — **Open in Browser** (HTML/Markdown/Notebook/JSON/SVG — `.md`,
   `.ipynb`, `.json` and `.jsonl` render via `chrome-extensions/local-viewer/`) and **Open in Default
...
`.htm` only. `EXTENSIONS` adds `.md`/`.markdown`, `.ipynb`, `.json`/`.jsonl`
and `.svg` by the same test each time — the browser is the only thing that
shows the file the way it is meant to look, and nothing else in either menu
does. `.pdf` fails that test: **Open in Default Application** already hands it
to whatever the machine uses to read PDFs. So do `.csv` and `.xml`, which a
browser renders no better than the editor does.
...
arrays and base64 blobs in that view. `.json` and `.jsonl` used to sit on the
other side of the line, because a browser showed them no better than the
editor; they crossed it when `chrome-extensions/local-viewer/` learned to
render them as a collapsible tree with an outline of the top-level keys — which
for a large file is exactly what the editor's flat text cannot give.
