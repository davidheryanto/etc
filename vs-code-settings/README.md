...
| File | What it does |
|------|--------------|
| `clutter-free.json` | Clean baseline, **lighter** preset: drops static visual noise (minimap, breadcrumbs, command center, line/occurrence highlights, SCM badge, the auto-opening right-hand chat panel) with no loss of function. Autocomplete and error-checking stay on. Pairs with `session-restore.json`. |
